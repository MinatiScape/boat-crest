...
public abstract class zzdf<E> extends zzdc<E> implements List<E>, RandomAccess {
    private static final zzdq<Object> zzlr = new x(z.zzlx, 0);

    public static <E> zzdf<E> zza(Object[] objArr) {
        int length = objArr.length;
        if (length == 0) {
            return (zzdf<E>) z.zzlx;
        }
        return new z(objArr, length);
    }
            ...
            if (size == list.size()) {
                if (list instanceof RandomAccess) {
                    for (int i = 0; i < size; i++) {
                        if (zzco.equal(get(i), list.get(i))) {
                        }
                    }
                    return true;
                }
