...
public class Detector {
    ...
    public final BitMatrix f11879a;
    public ResultPointCallback b;

    public Detector(BitMatrix bitMatrix) {
        this.f11879a = bitMatrix;
    }

    public static int b(ResultPoint resultPoint, ResultPoint resultPoint2, ResultPoint resultPoint3, float f) throws NotFoundException {
    ...
    public static PerspectiveTransform c(ResultPoint resultPoint, ResultPoint resultPoint2, ResultPoint resultPoint3, ResultPoint resultPoint4, int i) {
        float x;
        float y;
        float f;
        float f2 = i - 3.5f;
    ...
    public static BitMatrix d(BitMatrix bitMatrix, PerspectiveTransform perspectiveTransform, int i) throws NotFoundException {
    ...
    public final float a(ResultPoint resultPoint, ResultPoint resultPoint2) {
        float f = f((int) resultPoint.getX(), (int) resultPoint.getY(), (int) resultPoint2.getX(), (int) resultPoint2.getY());
        float f2 = f((int) resultPoint2.getX(), (int) resultPoint2.getY(), (int) resultPoint.getX(), (int) resultPoint.getY());
        return Float.isNaN(f) ? f2 / 7.0f : Float.isNaN(f2) ? f / 7.0f : (f + f2) / 14.0f;
    }
    ...
    public final float e(int i, int i2, int i3, int i4) {
        ...
        while (true) {
            ...
            if (z2 == detector.f11879a.get(i19, i20)) {
                if (i18 == 2) {
    ...
    public final float f(int i, int i2, int i3, int i4) {
        ...
        if (i5 < 0) {
            f = i / (i - i5);
            i5 = 0;
        } else if (i5 >= this.f11879a.getWidth()) {
            f = ((this.f11879a.getWidth() - 1) - i) / (i5 - i);
            i5 = this.f11879a.getWidth() - 1;
        } else {
            f = 1.0f;
        }
        float f3 = i2;
        ...
        if (i7 < 0) {
            f2 = f3 / (i2 - i7);
        } else if (i7 >= this.f11879a.getHeight()) {
            f2 = ((this.f11879a.getHeight() - 1) - i2) / (i7 - i2);
            i6 = this.f11879a.getHeight() - 1;
        } else {
            i6 = i7;
            f2 = 1.0f;
        }
        return (e + e(i, i2, (int) (i + ((i5 - i) * f2)), i6)) - 1.0f;
    }

    public final AlignmentPattern findAlignmentInRegion(float f, int i, int i2, float f2) throws NotFoundException {
        int i3 = (int) (f2 * f);
        int max = Math.max(0, i - i3);
        int min = Math.min(this.f11879a.getWidth() - 1, i + i3) - max;
        float f3 = 3.0f * f;
        if (min >= f3) {
            int max2 = Math.max(0, i2 - i3);
            int min2 = Math.min(this.f11879a.getHeight() - 1, i2 + i3) - max2;
            if (min2 >= f3) {
                return new a(this.f11879a, max, max2, min, min2, f, this.b).c();
            }
            throw NotFoundException.getNotFoundInstance();
        }
        throw NotFoundException.getNotFoundInstance();
    ...
    public final DetectorResult processFinderPatternInfo(FinderPatternInfo finderPatternInfo) throws NotFoundException, FormatException {
        ...
        if (calculateModuleSize >= 1.0f) {
            int b = b(topLeft, topRight, bottomLeft, calculateModuleSize);
            Version provisionalVersionForDimension = Version.getProvisionalVersionForDimension(b);
            ...
            return new DetectorResult(d(this.f11879a, c(topLeft, topRight, bottomLeft, alignmentPattern, b), b), alignmentPattern == null ? new ResultPoint[]{bottomLeft, topLeft, topRight} : new ResultPoint[]{bottomLeft, topLeft, topRight, alignmentPattern});
        }
        throw NotFoundException.getNotFoundInstance();
    }
    ...
    public final DetectorResult detect(Map<DecodeHintType, ?> map) throws NotFoundException, FormatException {
        ...
        return processFinderPatternInfo(new FinderPatternFinder(this.f11879a, this.b).e(map));
    }
}
