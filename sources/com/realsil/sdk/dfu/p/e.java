...
public final class e extends c {
    ...
    public void a(ExtendedBluetoothDevice extendedBluetoothDevice) {
        ...
        if (this.mProcessState == 519) {
            ...
            ZLogger.v(this.DBG, parseFromBytes.toString());
            ...
            boolean z = this.VDBG;
            ZLogger.v(z, "manufacturerSpecificData=" + DataConverter.bytes2Hex(manufacturerSpecificData));
            boolean z2 = this.VDBG;
            ZLogger.v(z2, "deviceMac= " + DataConverter.bytes2Hex(getOtaDeviceInfo().getDeviceMac()));
            equals = Arrays.equals(manufacturerSpecificData, getOtaDeviceInfo().getDeviceMac());
        } else if (this.mProcessState == 515) {
            String str = this.mDeviceAddress;
            equals = str != null && str.equals(device.getAddress());
        } else if (this.mProcessState != 527) {
            ZLogger.d("ignore process state: " + this.mProcessState);
            return;
        } else if (parseFromBytes == null) {
            ZLogger.d("ignore , specScanRecord is null");
            return;
        } else {
            ...
            boolean z3 = this.DBG;
            ZLogger.v(z3, "manufacturerSpecificData=" + DataConverter.bytes2Hex(manufacturerSpecificData2));
            boolean z4 = this.DBG;
            ZLogger.v(z4, "rwsBdAddr= " + DataConverter.bytes2Hex(getOtaDeviceInfo().getRwsBdAddr()));
            equals = Arrays.equals(manufacturerSpecificData2, getOtaDeviceInfo().getRwsBdAddr());
        }
        if (equals) {
            this.mOtaDeviceName = extendedBluetoothDevice.getName();
            this.mOtaDeviceAddress = device.getAddress();
            boolean z5 = this.DBG;
            ZLogger.d(z5, "find target device: name=" + this.mOtaDeviceName + " addr=" + this.mOtaDeviceAddress);
            q();
            synchronized (this.A) {
                this.B = true;
                this.A.notifyAll();
    ...
    public final void g(int i) throws DfuException {
        ...
        if (!this.mAborted) {
            ...
            ZLogger.w(String.format("0x%04X, Get target image info failed", Integer.valueOf((int) DfuException.ERROR_OPCODE_RESPONSE_NOT_SUPPORTED)));
