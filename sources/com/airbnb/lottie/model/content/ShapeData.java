...
public class ShapeData {
    ...
    public final List<CubicCurveData> f2045a;
    public PointF b;
    public boolean c;
    ...
    public void interpolateBetween(ShapeData shapeData, ShapeData shapeData2, @FloatRange(from = 0.0d, to = 1.0d) float f) {
        ...
        if (this.f2045a.size() < min) {
            for (int size = this.f2045a.size(); size < min; size++) {
                this.f2045a.add(new CubicCurveData());
            }
        } else if (this.f2045a.size() > min) {
            for (int size2 = this.f2045a.size() - 1; size2 >= min; size2--) {
                List<CubicCurveData> list = this.f2045a;
        ...
        for (int size3 = this.f2045a.size() - 1; size3 >= 0; size3--) {
            ...
            this.f2045a.get(size3).setControlPoint1(MiscUtils.lerp(controlPoint1.x, controlPoint12.x, f), MiscUtils.lerp(controlPoint1.y, controlPoint12.y, f));
            this.f2045a.get(size3).setControlPoint2(MiscUtils.lerp(controlPoint2.x, controlPoint22.x, f), MiscUtils.lerp(controlPoint2.y, controlPoint22.y, f));
            this.f2045a.get(size3).setVertex(MiscUtils.lerp(vertex.x, vertex2.x, f), MiscUtils.lerp(vertex.y, vertex2.y, f));
        }
    }
    ...
    public String toString() {
        return "ShapeData{numCurves=" + this.f2045a.size() + "closed=" + this.c + '}';
    }
