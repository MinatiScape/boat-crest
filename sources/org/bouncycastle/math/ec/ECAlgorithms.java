...
public class ECAlgorithms {
    ...
    public static ECPoint b(ECPoint eCPoint, BigInteger bigInteger, ECPoint eCPoint2, BigInteger bigInteger2) {
        ...
        return d(z ? precompute.getPreCompNeg() : precompute.getPreComp(), z ? precompute.getPreComp() : precompute.getPreCompNeg(), WNafUtil.generateWindowNaf(max, abs), z2 ? precompute2.getPreCompNeg() : precompute2.getPreComp(), z2 ? precompute2.getPreComp() : precompute2.getPreCompNeg(), WNafUtil.generateWindowNaf(max2, abs2));
    }
    ...
    public static ECPoint g(boolean[] zArr, WNafPreCompInfo[] wNafPreCompInfoArr, byte[][] bArr) {
    ...
    public static ECPoint h(ECPoint[] eCPointArr, BigInteger[] bigIntegerArr, GLVEndomorphism gLVEndomorphism) {
        ...
        if (gLVEndomorphism.hasEfficientPointMap()) {
            return e(eCPointArr, pointMap, bigIntegerArr2);
        }
            ...
            if (eCPointArr.length >= 1) {
                ...
                if (length != 1) {
                    if (length != 2) {
                        ...
                        return endomorphism instanceof GLVEndomorphism ? validatePoint(h(eCPointArr2, bigIntegerArr, (GLVEndomorphism) endomorphism)) : validatePoint(f(eCPointArr2, bigIntegerArr));
                    }
    ...
    public static ECPoint sumOfTwoMultiplies(ECPoint eCPoint, BigInteger bigInteger, ECPoint eCPoint2, BigInteger bigInteger2) {
        ...
        if ((curve instanceof ECCurve.AbstractF2m) && ((ECCurve.AbstractF2m) curve).isKoblitz()) {
            h = eCPoint.multiply(bigInteger).add(importPoint.multiply(bigInteger2));
        } else {
            ECEndomorphism endomorphism = curve.getEndomorphism();
            h = endomorphism instanceof GLVEndomorphism ? h(new ECPoint[]{eCPoint, importPoint}, new BigInteger[]{bigInteger, bigInteger2}, (GLVEndomorphism) endomorphism) : b(eCPoint, bigInteger, importPoint, bigInteger2);
        }
        return validatePoint(h);
    }
