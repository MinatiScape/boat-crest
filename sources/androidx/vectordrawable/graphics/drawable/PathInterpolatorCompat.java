...
public class PathInterpolatorCompat implements Interpolator {
    ...
    public final void b(Path path) {
        ...
        if (min > 0) {
            this.f1733a = new float[min];
            this.b = new float[min];
            float[] fArr = new float[2];
            for (int i2 = 0; i2 < min; i2++) {
                pathMeasure.getPosTan((i2 * length) / (min - 1), fArr, null);
                this.f1733a[i2] = fArr[0];
                this.b[i2] = fArr[1];
            }
            if (Math.abs(this.f1733a[0]) <= 1.0E-5d && Math.abs(this.b[0]) <= 1.0E-5d) {
                int i3 = min - 1;
                if (Math.abs(this.f1733a[i3] - 1.0f) <= 1.0E-5d && Math.abs(this.b[i3] - 1.0f) <= 1.0E-5d) {
                    float f = 0.0f;
                    int i4 = 0;
                    while (i < min) {
            ...
            sb.append(this.f1733a[0]);
            sb.append(Constants.SEPARATOR_COMMA);
            sb.append(this.b[0]);
            sb.append(" end:");
            int i6 = min - 1;
            sb.append(this.f1733a[i6]);
            sb.append(Constants.SEPARATOR_COMMA);
            sb.append(this.b[i6]);
    ...
    public float getInterpolation(float f) {
        ...
        int i = 0;
        int length = this.f1733a.length - 1;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f1733a[i2]) {
                length = i2;
