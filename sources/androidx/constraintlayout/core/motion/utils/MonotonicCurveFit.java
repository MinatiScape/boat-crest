...
public class MonotonicCurveFit extends CurveFit {
    ...
    public void getPos(double d, double[] dArr) {
        double[] dArr2 = this.f881a;
        int length = dArr2.length;
        int i = 0;
        int length2 = this.b[0].length;
        if (this.d) {
            if (d <= dArr2[0]) {
                getSlope(dArr2[0], this.e);
                for (int i2 = 0; i2 < length2; i2++) {
                    dArr[i2] = this.b[0][i2] + ((d - this.f881a[0]) * this.e[i2]);
                }
                return;
            }
            int i3 = length - 1;
            if (d >= dArr2[i3]) {
                getSlope(dArr2[i3], this.e);
                while (i < length2) {
                    dArr[i] = this.b[i3][i] + ((d - this.f881a[i3]) * this.e[i]);
                    i++;
                }
                return;
            }
        ...
        while (i6 < length - 1) {
            ...
            if (d < dArr3[i8]) {
                ...
                while (i < length2) {
                    ...
                    double[][] dArr5 = this.c;
                    dArr[i] = c(d2, d3, d4, d5, dArr5[i6][i], dArr5[i8][i]);
                    i++;
                }
                return;
            }
    ...
    public void getPos(double d, float[] fArr) {
        ...
        if (this.d) {
            if (d <= dArr[0]) {
                getSlope(dArr[0], this.e);
                for (int i2 = 0; i2 < length2; i2++) {
                    fArr[i2] = (float) (this.b[0][i2] + ((d - this.f881a[0]) * this.e[i2]));
                }
                return;
            }
            int i3 = length - 1;
            if (d >= dArr[i3]) {
                getSlope(dArr[i3], this.e);
                while (i < length2) {
                    fArr[i] = (float) (this.b[i3][i] + ((d - this.f881a[i3]) * this.e[i]));
                    i++;
                }
                return;
            }
        ...
        while (i6 < length - 1) {
            ...
            if (d < dArr2[i8]) {
                ...
                while (i < length2) {
                    ...
                    double[][] dArr4 = this.c;
                    fArr[i] = (float) c(d2, d3, d4, d5, dArr4[i6][i], dArr4[i8][i]);
                    i++;
                }
                return;
            }
    ...
    public double getPos(double d, int i) {
        ...
        while (i2 < length - 1) {
            ...
            if (d < dArr2[i5]) {
                ...
                double[][] dArr4 = this.c;
                return c(d2, d3, d4, d5, dArr4[i2][i], dArr4[i5][i]);
            }
            i2 = i5;
        }
        return 0.0d;
